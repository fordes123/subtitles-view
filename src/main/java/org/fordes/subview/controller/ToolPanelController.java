...
public class ToolPanelController extends RootController implements Initializable {
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        String temp= startController.inputset.getCode().equals("GBK")?"ANSI（GBK）": startController.inputset.getCode();
    ...
    public void Call(TextArea TargetEditor,String type){
        ...
        switch (type){
            ...
            case "code":
                ...
                code_box.getSelectionModel().select(startController.inputset.getCode().equals("GBK")?"ANSI（GBK）": startController.inputset.getCode());
                code= startController.inputset.getCode();
            ...
            case "style":
                ...
                Style_FontList.getSelectionModel().selectedItemProperty().addListener(new ChangeListener() {
                    @Override
                    public void changed(ObservableValue observable, Object oldValue, Object newValue) {
                        startController.getSettings().getBaseSettings().put("Editor_Font",newValue.toString());
                        editor.setFont(new Font(newValue.toString(),Integer.valueOf(startController.getSettings().getBaseSettings().get("Editor_Size"))));
                    }
                });
                ...
                Style_FontSizeList.getSelectionModel().selectedItemProperty().addListener(new ChangeListener() {
                    @Override
                    public void changed(ObservableValue observable, Object oldValue, Object newValue) {
                        startController.getSettings().getBaseSettings().put("Editor_Size",newValue.toString());
                        editor.setFont(new Font(startController.getSettings().getBaseSettings().get("Editor_Font"),(double)Integer.valueOf(newValue.toString())));
                    }
                });
                break;
        }
    ...
    public void Style_default(ActionEvent actionEvent) {
        if(!Style_FontList.getSelectionModel().getSelectedItem().toString().equals("System")||!Style_FontSizeList.getSelectionModel().getSelectedItem().toString().equals("18")) {
            ...
            startController.getSettings().getBaseSettings().put("Editor_Font","System");
            startController.getSettings().getBaseSettings().put("Editor_Size","18");
            editor.setFont(Font.font("System",(double) 18.0));
            editor.requestFocus();
            new ToastUtil().toast(startController.inputset.getStage(),"已恢复默认样式", startController.inputset.getTheme(),1000);
            return;
        }
    }
    ...
    public void onFind(ActionEvent actionEvent) {
        ...
        if(!fr.search()) {
            new ToastUtil().toast(startController.inputset.getStage(), "未找到", startController.inputset.getTheme(), 1000);
            editor.selectRange(0,0);
            find_input.requestFocus();
    ...
    public void Replace_all(ActionEvent actionEvent) {
        ...
        if(!fr.replaceAll(ReplaceTarget)) {
            ...
            new ToastUtil().toast(startController.inputset.getStage(), "已替换全部", startController.inputset.getTheme(), 1000);
            editor.selectRange(0,0);
            Replace_replace_input.requestFocus();
            Replace_replace_input.selectAll();
        }
    ...
    public void onJumpLine(ActionEvent actionEvent) {
        ...
        if(number.equals("")||number==null||!isNum.matches()||Integer.parseInt(number)==0) {
            Jump_input.setText("");
            new ToastUtil().toast(startController.inputset.getStage(),"输入无效！", startController.inputset.getTheme(),500);
            return;
        }
        ...
        if(position!=-1){
            ...
            Jump_input.requestFocus();
            new ToastUtil().toast(startController.inputset.getStage(),"未找到此行", startController.inputset.getTheme(),500);
            return;
        }
   ...
   /* 编码改变时刷新文本区*/
    public void Refresh(ActionEvent actionEvent) {
        startController.inputset.setCode(code);
        if(startController.inputset.getSubtitles().getFile()!=null)
            editor.setText(new SubFileUtil().Read(startController.inputset.getSubtitles().getFile(), startController.inputset.getCode()));
        new ToastUtil().toast(startController.inputset.getStage(),"已更改编码，如出现问题，请还原至原选项", startController.inputset.getTheme(),1000);
