...
public class BuildPanelController extends RootController implements Initializable {
    @FXML
    ...
    public void onBuild_Manual(ActionEvent actionEvent) {
        ...
        if(startController.focus_indicator.equals(Build_Manual.getId()))
            return;
        startController.focus_indicator=Build_Manual.getId();//设置焦点
        hide();
        //初始化
        ...
        Build_Mode.getSelectionModel().selectedItemProperty().addListener(new ChangeListener() {
            ...
            public void changed(ObservableValue observable, Object oldValue, Object newValue) {
                if(((String)Build_Mode.getValue()).equals("合成视频")){
                    if(startController.inputset.getVideo().getFile()==null) {//打开的文件为字幕，且并未选择视频
        ...
        Build_OutPath.setText(((String)Build_Mode.getValue()).equals("合成视频")? startController.inputset.getVideo().getFile().getParent(): startController.inputset.getSubtitles().getFile().getParent());

        String temp = Build_Mode.getValue().toString().equals("合成视频")? startController.inputset.getVideo().getFile().getName().substring(0, startController.inputset.getVideo().getFile().getName().lastIndexOf(".")): startController.inputset.getSubtitles().getFile().getName().substring(0, startController.inputset.getSubtitles().getFile().getName().lastIndexOf("."));
    ...
    public void onBuild_Preview(ActionEvent actionEvent) {
        ...
        //询问焦点，如已在此选项上，则操作无效化，避免重复加载
        if(startController.focus_indicator.equals(Build_Preview.getId()))
            return;
        startController.focus_indicator=Build_Preview.getId();//设置焦点
        hide();
    ...
    public void onStartBuild(ActionEvent actionEvent) throws IOException {
        ...
        if(((String)Build_Mode.getValue()).equals("合成视频")){
            if(startController.inputset.getSubtitles().getFile()==null) {//以视频开始进入程序，先生成字幕文件
                File tempSubtitle=new File(new ConfigPathUtil().getTempPath() + "\\tempSubtitle.srt");
                if(tempSubtitle.exists())//检测文件是否存在，若已经存在，删除之
                    tempSubtitle.delete();
                new SubFileUtil().GenerateSubtitles(startController.inputset.getText(),tempSubtitle,1,null,true);//生成文件
                startController.inputset.setSubtitles(new FileSet(tempSubtitle,1));
            }
            input= startController.inputset.getVideo().getFile().getPath();
            filePath= startController.inputset.getSubtitles().getFile().getPath();
            oldFilePath= startController.inputset.getVideo().getFile().getPath();
            ...
            oldFilePath= startController.inputset.getVideo().getFile()==null? startController.inputset.getSubtitles().getFile().getPath(): startController.inputset.getVideo().getFile().getPath();
            task= new Task() {
                @Override
                protected Object call() throws Exception {
                    ...
                    new SubFileUtil().GenerateSubtitles(startController.inputset.getText(),tempSubtitle, startController.inputset.getSubtitles().getType(),coding,false);
                    updateMessage("done");
                    return true;
                }
            };
        ...
        new LoadUtil().load(startController.inputset.getStage(), startController.inputset.getTheme(),task,last,"处理中");
        return;
    }
