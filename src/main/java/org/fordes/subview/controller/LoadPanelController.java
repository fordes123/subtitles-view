...
import org.fordes.subview.util.VoiceUtil.lfasrVoiceUtil;

import java.io.File;
import java.net.URL;
import java.util.ResourceBundle;

public class LoadPanelController extends RootController implements Initializable {
    @FXML
    ...
    private void createWork(){
        ...
        work= new Task() {
            ...
            protected Object call() throws Exception {
                ...
                new FFMpegUtil().SegmentedAudio(startController.inputset.getVideo().getFile().getPath(), audio.getPath());//提取音轨

                updateMessage("开始语音转换，请保持联机...");
                updateProgress(0.6, 1);
                String res=new lfasrVoiceUtil(audio).start(getLanguage(),getSpeakersNumber());
                if(res==null)
                    return false;
                updateMessage("转换完成，正在解析结果...");
                updateProgress(0.9, 1);
                startController.inputset.setText(new TimelineUtil(1).parsing(res).toString());
        ...
        last= new Task(){
            ...
            protected Object call() throws Exception {
                Platform.runLater(new Runnable() {
                    ...
                    public void run() {
                        try {
                            new mainApplication().start(startController.inputset.getStage());
                            top.close();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
