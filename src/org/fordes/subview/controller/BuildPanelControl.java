    ...
    public void onStartBuild(ActionEvent actionEvent) throws IOException {
        ...
        if(((String)Build_Mode.getValue()).equals("合成视频")){
            if(controller.inputset.getSubFile()==null) {//以视频开始进入程序，先生成字幕文件
                File tempSubtitle=new File(System.getProperty("user.dir") + "\\date\\tempSubtitle.srt");
            ...
            //input=controller.inputset.getFile().getPath();
            //创建任务
            task= new Task() {
                @Override
                protected Object call() throws Exception {
                    ...
                    //执行命令
                    new SubFileUtil().GenerateSubtitles(controller.inputset.getText(),tempSubtitle,controller.inputset.getSubType(),coding,false);//生成文件
                    updateMessage("done");
                    return true;
                }
            };
            ...
            protected Object call() throws Exception {
                Platform.runLater(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(Build_End.isSelected()){//结束后退出程序
